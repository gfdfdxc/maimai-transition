...
| `--dur-base` | **0.55s** | 底板（含小方块） |
| `--dur-purple` | **0.55s** | 两角淡紫，delay **0.03s** |
| `--dur-white` | **0.55s** | 两角白色，delay **0.06s** |
| `--dur-bg` | **0.55s** | slide/hold 默认（jitter 后 0.40-0.70s） |
...
| **v11（当前）** | **-180°→0°（真·CCW，半圈）** | 4.50 | 0.18s | 0.78s | **关键修正：CSS rotate 角度递减实际是 CW，必须用负角度才是 CCW**；同时缩短 dur-base/purple/white 0.78→0.55s 让角组在 ~500ms 内到位（用户反馈"偏晚"）；delay 微调 0.05/0.10 → 0.03/0.06 |
