...
| **v46** | 同 v41 | 同 v41 | 1.62s | 同 v44 | 用户指出：底板出场不应直接淡出，应像入场一样飞向角落。<br>**纠正 v43 过度简化**：删除 `#底板` 父组 `bg-out-expand` scale，恢复 `expand-TL/BR` 子层关键帧：左上方块 (0,0)→(-2400,-1500)、右下方块 (0,0)→(2400,1500)（与入场 settle-TL/BR 完全对称），delay 1.746s / dur 0.294s。出场时左上、右下方块分别飞回角落外，2.04s 同时退完 |
| **v47（当前）** | 同 v41 | 同 v41 | 1.62s | 同 v44 / Slide-out 提早 | 用户指出 slide 出场太晚。对比参考视频 f95(1.583s) slide 已开始退、~f105(1.75s) 退完。<br>jitterSlides JS: `delayOut 1.746→1.583s (+jitter 0.04)`、`durOut 0.27-0.31→0.15-0.18s`。HOLD/Slide 现在与 chip 收缩(1.62s) 平行启动退场，到 chip 开始放大(1.746s)时已大部分退完。底板/两角 出场时序保持不变 |
