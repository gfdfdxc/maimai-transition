...
| **v18（当前）** | -180°→0°（真·CCW，半圈） | 4.50 | t0+0.10s | 0.70s | **全局 170ms 起始延迟修复"动画过快"**（用户反馈："为什么我现在感觉整个入场速度那么快"）：对照参考视频密集帧栅格（每 3 帧一张），发现 **f3/f6/f9 完全空白，动画从 f10~f12 才开始**。所以新增 CSS var `--t0: 0.17s`，所有元素的 animation-delay 加上 t0；chip delay = t0 + 0.10s；jitterSlides 注入 delay 改为 0.17 + 0~0.06s |
