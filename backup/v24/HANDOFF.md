...
| **v43（当前）** | 同 v41 | 同 v41 | 1.62s | 同 chip（1.62s + dur 0.42s） | 用户纠正："chip 收缩时左上/右下子层也跟着收缩；chip 放大时子层+背景也跟着放大"。<br>实现：新增 `bg-out` keyframes (scale 1→0.94→1.90→2.90→3.70→4.20, op 1→1→1→0.55→0.15→0)；`#底板, #两角淡紫, #两角白色` 父组挂 bg-out（transform-box: view-box; origin 50% 50%）与 chip-out 同步起停；子组旧 expand-TL/BR、converge-TL-out/BR-out 改为保留稳态的空 keyframes（父组负责整体出场缩放）。SLIDE/HOLD 仍走原 45° 飞出（装饰条独立行为） |
