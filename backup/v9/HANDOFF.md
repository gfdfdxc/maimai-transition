...
角组 keyframes（v14，**重新加回回弹 · 但不越过终点向中心**）：
```
converge-TL: 0%(-2400,-1500) → 70%(0,0) → 85%(-40,-24) → 100%(0,0)
converge-BR: 0%(+2400,+1500) → 70%(0,0) → 85%(+40,+24) → 100%(0,0)
timing-function: linear  // 由关键帧控制节奏
```

> ⚠️ **回弹方向限制**：角元素达到终点后，只能向“来的方向”（左上组往左上、右下组往右下）反弹一下，**绝不能越过终点向画面中心振荡** — 否则元素远端边缘会从屏幕外缘缩回，露出网页黑色背景。v12 均量过冲造成露背景，v13 取消回弹但用户要求保留，v14 采用“反向振荡”。
...
| **v14（当前）** | -180°→0°（真·CCW，半圈） | 4.50 | 0.10s | 0.70s | **重新加回角组回弹，但反向振荡**（用户："我需要回弹，只是想让你帮我解决回弹会显示背景或底色的问题"）：converge-TL/BR 变为 0%→70%(0,0)→85%(⁄40px,⁄24px)→100%(0,0)，即“到终点后反跳向左上/右下”而不是“冲过终点到中心”，这样远端边缘始纶覆盖屏幕外不露背景；timing-function 改回 `linear` 由关键帧控制节奏 |
