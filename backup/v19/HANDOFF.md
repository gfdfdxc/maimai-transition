...
| **v28（当前）** | 同上 | 同上 | 同上 | 同上 | 微调最终：t-exit-base 1.87→1.83s、t-exit-bg 1.88→1.84s。与参考视频 col1–col6 (f95/f105/f112/f117/f120/f125) 几乎完全同步，仅 col4 chip 圆环偏清晰 ≈30ms |

## 8. 下一步建议

v28 出场动画已与参考视频几乎同步。参数快查：

| 元素 | 起点 (s) | dur (s) | 关键帧要点 |
|------|----------|---------|-----------|
| chip-out | 1.58 | 0.45 | 0→37%(scale 0.78 缩)→63%(2.4)→85%(3.4, op 0.25)→100%(4.0, op 0) 无旋转 |
| 底板 / 两角 | 1.83 | 0.22 | 0→15%(内收 30/18)→100%(飞出角外) |
| HOLD / SLIDE | 1.84+jitter | 0.20-0.28 | 从稳态(0,0)→21opacity 0+继续原方向飞出 |
