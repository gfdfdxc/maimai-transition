...
角组 keyframes（v13，**取消 overshoot**）：
```
converge-TL: 0%(-2400,-1500) → 100%(0,0)
converge-BR: 0%(+2400,+1500) → 100%(0,0)
timing-function: cubic-bezier(0.22, 1, 0.36, 1)  // easeOutCubic 等价
```

> ⚠️ **为什么取消 overshoot**：之前 60% 关键帧让元素冲过最终位置往中心挤压 100/60 px，导致元素"远端"边缘（覆盖屏幕外缘的部分）从屏幕边缘缩回 100px，**露出网页黑色背景 + 中间白色**。改用纯 ease-out 单调缓出后，元素从远处到最终位置过程中"远端"边缘始终覆盖到屏幕外，不会露背景。
...
| **v13（当前）** | -180°→0°（真·CCW，半圈） | 4.50 | 0.10s | 0.70s | **角组移除 overshoot 改 ease-out**（用户反馈："角组 overshoot 越过最终位置时，远端边缘从屏幕边缘缩回，露出网页黑色底色 + 中间白色"）：converge-TL/BR 由 4 关键帧（含 60%、82% overshoot/回弹）简化为 0%/100% 两帧；timing-function 从 `linear` 改为 `cubic-bezier(0.22, 1, 0.36, 1)` 保持"由快到慢"的视觉感 |
