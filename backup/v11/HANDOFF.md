...
| **v17（当前）** | -180°→0°（真·CCW，半圈） | 4.50 | 0.10s | 0.70s | **两角 + 底板都提前到位**（用户反馈："左上/右下子层入场时间太晚"，对照参考 f15=250ms 已接近稳态）：两角 dur 0.77→0.55s + delay 取消、keyframes 60%/82% 提前到 40%/65%（即 220ms overshoot、357ms 回弹谷值，与参考节奏吻合）；底板 settle-TL/BR keyframes 改为 0%→30%→100%（30% 到位后静止），dur 0.80s 保持 end@800ms，30% × 0.80 = 240ms ≈ 参考 f15 已铺满 |
