...
| **v15（当前）** | -180°→0°（真·CCW，半圈） | 4.50 | 0.10s | 0.70s | **底板取消回弹 + 两角恢复 v12 回弹 + scale(1.06) 向远端放大**（用户：不好，恢复到 v12。可否加大白小三角和粉大三角使他们往左上/右下放大，不要把实际在动画中显示的边放大；两个底板不要应用回弹即可）：新增 `settle-TL/BR` keyframes （0%→100% 单调 ease-out）用于底板；converge-TL/BR 恢复 v12 回弹 (60%/82%/100%)，且复合 `translate(1920,1080) scale(1.06) translate(-1920,-1080)` 以画面中心为锚点放大 6%，让两角三角朝远端扩 ~115/65 px > 回弹偏移 100/60 px，保证不露背景；三角朝中心的“显示边”几乎不变 |
