...
| **v30（当前）** | 同上 | 同上 | 同上 | 同上 | v29 退场反而过快（f120 参考仍有大量背景，我方已退完）。调整：t-exit-base 1.83→1.88s、dur 0.22→0.28s；SLIDE/HOLD dur 0.18-0.22 → 0.24-0.28、delay 1.83+0.02 → 1.88+0.02。所有元素最晚于 ~2.18s 全完成，与参考 f135 全空对齐 |

## 8. 下一步建议

v30 出场动画已与参考视频几乎同步。参数快查：

| 元素 | 起点 (s) | dur (s) | 关键帧要点 |
|------|----------|---------|-----------|
| chip-out | 1.58 | 0.45 | 0→37%(scale 0.78 缩)→63%(2.4)→85%(3.4, op 0.25)→100%(4.0, op 0) 无旋转 |
| 底板 / 两角 | 1.88 | 0.28 | 0→15%(内收 30/18)→100%(飞出角外) |
| HOLD / SLIDE | 1.88+jitter | 0.24-0.28 | 从稳态(0,0)→opacity 0+继续沿 45° 对角飞出 (±2200,∓2200) |

下一步可选方向：

1. **微调 chip 后段淡出（可选，f117 圆环偏清 ~30ms）**：85% 关键帧 op 0.25 → 0.15 让 f117 chip 更透明
2. **加播放控制**：进度条、暂停、单帧步进，便于逐帧调试
3. **优化入场/出场衡接**：检查 f55-f95 稳态期间是否有微动（目前是完全静止）
